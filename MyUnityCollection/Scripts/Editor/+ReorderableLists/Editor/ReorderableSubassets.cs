...
namespace Muc.Editor.Internal {
  ...
  internal class ReorderableSubassets : ReorderableStructures {

    private readonly SerializedObjectCache serializedObjectCache = new SerializedObjectCache();
    ...
    public ReorderableSubassets(ReorderableAttribute attribute, SerializedProperty property, Type listType, Type elementType, Type[] subassetTypes)
      ...
      useFullSubassetTypeNames = SubassetTypeNamesAreAmbiguous();
    ...
    protected override void PopulateElementContextMenu(GenericMenu menu, int elementIndex) {
      foreach (var mutableElementType in subassetTypes) {
        var elementType = mutableElementType;

        var elementTypeName =
          useFullSubassetTypeNames
        ...
        menu.AddItem(new GUIContent(insertAbove), false, () => OnNextGUIFrame(() => InsertSubasset(elementType, elementIndex)));
        menu.AddItem(new GUIContent(insertBelow), false, () => OnNextGUIFrame(() => InsertSubasset(elementType, elementIndex + 1)));
      }
      menu.AddSeparator("");
    ...
    private void DrawElementHeader(Rect position, Object subasset, bool isActive) {
      ...
      titleContent.image = EditorGUIUtility.ObjectContent(subasset, subassetType).image;
    ...
    private bool OnCanAddCallback(ReorderableList list) {
      return subassetTypes.Length > 0;
    }

    private void OnAddCallback(ReorderableList list) {
      serializedProperty.isExpanded = true;
      AddSubasset(subassetTypes[0]);
    }
    ...
    private void InsertSubasset(Type subassetType, int elementIndex) {
      ...
      if (subasset == null) {
        Debug.LogErrorFormat("Failed to create subasset of type {0}", subassetType.FullName);
        return;
      }
    ...
    private bool SubassetTypeNamesAreAmbiguous() {
      var elementTypeNames = subassetTypes.Select(t => t.Name);
      var elementTypeNamesAreAmbiguous = elementTypeNames.Count() > elementTypeNames.Distinct().Count();
    ...
    private SerializedObject GetSerializedObjectFromCache(Object @object) {
      var cache = serializedObjectCache;
    ...
    private void EvictObsoleteSerializedObjectsFromCache() {
      ...
      if (destroyedObjects.Any()) {
        foreach (var destroyedObject in destroyedObjects.ToArray()) {
          cache.Remove(destroyedObject);
        }
      }
    }
