...
namespace Unitylity.Data.Trees {
	...
	public partial class VoxelTree<T> {
		...
		public class Enumerator : ITreeEnumerator<T> {
			...
			private class Tracker : Tracker<VoxCell<T>> {
				public Tracker(VoxCell<T> cell, int i = 0) : base(cell, i) { }
			}
		...
		public class DetailedEnumerator : ITreeEnumerator<T> {
			...
			private class Tracker : Tracker<VoxCell<T>> {
				...
				public float size;
				public Tracker(VoxCell<T> cell, Vector3 origin, float size, int i = 0) : base(cell, i) {
					this.origin = origin;
					this.size = size;
				}
			}
